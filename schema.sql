...
-- netlify/functions/data.js (org, provider, patient, appt, encounter, claim,
-- payer, master, task, credentialing, history, document, payerenrolment)
-- reads and writes through this single polymorphic table rather than the
-- per-entity tables above. Both must exist; the per-entity tables are kept
-- for other functions/reports that query them directly.
...
CREATE INDEX IF NOT EXISTS app_records_search_idx  ON app_records (kind, search)      WHERE deleted_at IS NULL;

-- §164.312(b): every read/write of a PHI-bearing kind through data.js is
-- logged here. Append only, same as audit_log.
...
CREATE INDEX IF NOT EXISTS phi_log_at_idx      ON phi_access_log (at DESC);
CREATE INDEX IF NOT EXISTS phi_log_patient_idx ON phi_access_log (patient_ref, at DESC);
